  ...
  document.querySelector(".pin-prompt form").addEventListener(
    ...
    function (event) {
      ...
      fetch(
        ...
        .then(({auth, exhausted}) => {
          if (auth) {
            ...
            fadeOut(document.getElementsByClassName("pin-prompt")[0]);
				...
				(() => {
					const innerDiv = document.querySelector('.pin-prompt .inner');
					const wrongText = document.createElement('h3');
					wrongText.innerText = "WRONG";
					wrongText.style.color = 'red';
					innerDiv.appendChild(wrongText);
					setTimeout(() => {
						innerDiv.removeChild(wrongText);
					}, 2000);
				})();
        ...
        .catch((err) => {
          alert("Error: Could not verify PIN...  Network error? Another nuke?");
          console.error(err);
        })
